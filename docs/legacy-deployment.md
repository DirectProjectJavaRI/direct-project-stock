...
The legacy model consists of Apache Tomcat running alongside Apache James to deploy the HISP, except for the DNS server, which runs as
...
For HISPs upgrading from an older version of the Java Reference Implementation, this deployment model is likely preferable for ease
...
The stock assembly contains all of the pre-assembled and configured components of the Bare Metal deployment. Download the latest version of the stock assembly from the [Maven Central repository](http://repo.maven.apache.org/maven2/org/nhind/direct-project-stock/6.0/direct-project-stock-6.0.tar.gz).

**Note:** Maven Central may blacklist some IP ranges, such as virtual machines running in the Amazon EC2 cloud. Use the Sonatype repository if you are blocked from the Maven Central repository.

The assembly contains a root directory named direct and has the following folders under the root.
...
Obtain the URL for the appropriate version of the assembly and download it to the /opt directory by running the wget command from the /opt directory. For example, to download version direct-project-stock-6.0.tar.gz from Maven Central, use the following commands:
...
Before running the James mail server, the configuration and message monitoring services must be running, and some minimum configuration must be completed. All of these services run inside the Tomcat web server.

To start the Tomcat server, run the following command from the DIRECT_HOME/apache-tomcat-9.0.17/bin directory.
...
The first step in running your mail server is configuring the domain and loading a trust anchor and certificate(s) into the configuration UI.

First, determine your HISP's domain name. Depending on the type of certificate resolution services you wish to host, your domain naming convention may differ slightly. Regardless of the certificate hosting model, you will need to have a registered domain. For this document, we will assume you have a domain registered called example.com.
...
The next step is to create a root certificate (anchor) for our domain and an X.509 certificate or pair of X.509 certificates for encrypting/decrypting and signing messages. There are many different options for getting these certificates, such as using OpenSSL or obtaining a certificate from a commercial third party such as DigiCert. However, the Direct Project reference implementation assembly ships with a tool called certGen for generating root CAs and certificates for the purpose of pilots and interop testing.

**NOTE:** The certificates generated by the certGen tool represent trust anchors and end entity certificates for domains and individual Direct addresses. However, these certificates do not implement a fully functional PKI (public key infrastructure), which would include a multitude of additional operational aspects. PKIs are generally implemented by third-party CAs such as DigiCert or VeriSign, but can be implemented by individual institutions if they have the resources to do so. PKI implementation is outside the scope of this document.
...
Now create a CA for your domain. In the certGen tool, enter a common name (CN:) for your new CA. For our domain direct.example.com, we might use something like Direct.Example.Com Root CA. Fill in the other fields as needed. It is recommended that you set the expiration to 1 year, the key strength to at least 2048 bits, and provide a password for your CA's private key.

After creating the CA, create a leaf cert using your domain name for the CN: field and fill in all other fields as needed. It is recommended that you set the expiration to at least 1 year, the key strength to at least 2048 bits, and provide a password for your private key. After creating your CA and certificate, you should have the following similarly named files in your /tools directory (assuming the direct.example.com domain and no email address entered in the CA dialog; if an email address is entered, the CA files will have the email address in the file name instead of the CN field).

* Direct.Example.com Root CA.der - Root CA file (trust anchor for your HISP)
...
The certGen tool supports generating single-use certificates. If you wish to implement single-use certificates, refer to this specific [section](single-use-certs).

## Import Anchors and Certificates

Before the security and trust agent can be run, you must create your domain in the configuration UI tool and import anchors and certificates. Follow the steps below to create your domain and import your trust anchor and certificate(s). A full description of the config UI and its operations can be found [here](https://directprojectjavari.github.io/gateway/SMTPWebConfiguration).
...
3. Click *Certificates* at the top of the screen
  * Click **Choose File** and browse to the location of your org certificate PKCS12 file, and select it
...
Before your HISP can communicate with other HISPs, you must import anchors from other HISPs to establish trusted communication. You must also provide your trust anchor to the HISP(s) you are communicating with. A few HISPs exist for interop testing that can be easily accessed. An easier way is to import trust bundles and add that trust bundle to your domain. A common test bundle is the DirectTrust Interop Test bundle found [here](https://bundles.directtrust.org/bundles/interopTestCommunity.p7b). This includes an anchor for the following test domain:
...
**NOTE:** The Windows version does not currently support running as a background service.
